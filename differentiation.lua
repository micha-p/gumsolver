...
function partial_iter (agent, y, x, starty, startx)
   ...
   if ITER or DEBUG then
      print2(warn("PARTIAL", PRINT16(y["name"]),PRINT16(x["name"]))) 
   end
   ...
   function calculate (agent, y, x)
      ...
      dydx=DIV(dy,dx)
      if ITER then
         print2(warn(n,PRINT16(dy),PRINT16(dx),PRINT16(dydx),"\t",PRINT16(yleft),PRINT16(yright),PRINT16(xleft),PRINT16(xright))) 
      end
      return dydx
   end
...
function partial_constraint (y,x,result)
  local me = {}
  local actors = {x, y, result}
  local agent = "partial"

  local function process_new_value ()
    if (not ITERLOOPP) and x.value() and y.value() and x.value()=="user" then 
       ITERLOOPP = not nil
       ...
       MUTE = mutestate
       x.set (origin, startx)
       result.set (agent, final)
       ITERLOOPP = nil
    end
  end
  local function process_forget_value ()
    if not x.value() or not y.value() then result.forget(me) end
  end
